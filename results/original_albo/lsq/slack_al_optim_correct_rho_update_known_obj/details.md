Ran Slack-AL + Optim on LSQ problem with correct penalty updates and *known* objective.
...
for(x in 1:100) {
  ## run ALBO
  out <- new_auglag(aimprob, B, start=5, end=50, slack=2, fhat=FALSE, lambda=0)
  write_json(out, glue("results/lsq/slack_al_optim_correct_rho_update_known_obj/data/run_{x}_results.json"), digits=NA)
}
