Ran Slack-AL + Optim on LSQ problem with correct handling of penalty updates.
...
for(x in 1:100) {
  ## run ALBO
  out <- new_auglag(aimprob, B, start=5, end=50, slack=2, fhat=TRUE, lambda=0)
  write_json(out, glue("results/lsq/slack_al_optim_correct_rho_update/data/run_{x}_results.json"), digits=NA)
}
