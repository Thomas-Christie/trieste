Results on GSBP problem with correct initialisation of penalty
and conservative reduction when no valid value has been found.
...
for(x in 1:50) {
  ...
  write_json(out, glue("results/gsbp/slack_al_optim_conservative_rho_update_urate_one/data/run_{x}_results.json"), digits=NA)
}
```