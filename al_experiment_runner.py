...
import numpy as np
import trieste
from trieste.acquisition.optimizer import generate_continuous_optimizer, generate_adam_optimizer, generate_random_search_optimizer
from trieste.acquisition.function.thompson_sampling_augmented_lagrangian import ThompsonSamplingAugmentedLagrangian
...
flags.DEFINE_string('save_path', 'results/final_ts_results/lockwood/dsdano_opt_rbf_uniform_random/data/run_',
...
def main(argv):
    ...
    for run in range(FLAGS.num_experiments):
        ...
        if FLAGS.problem == "LSQ" or FLAGS.problem == "LOCKWOOD":
            augmented_lagrangian = ThompsonSamplingAugmentedLagrangian(objective_tag=OBJECTIVE,
        ...
        if FLAGS.acquisition_fn_optimiser == 'l-bfgs-b':
            # optimizer = generate_continuous_optimizer(num_initial_samples=FLAGS.num_acquisition_optimiser_start_points,
            #                                           num_optimization_runs=2)
            optimizer = generate_continuous_optimizer(num_initial_samples=FLAGS.num_acquisition_optimiser_start_points,
                                                      num_optimization_runs=2, optimizer_args={"options": {"maxls": 100}})
            ...
            optimizer = generate_adam_optimizer(num_initial_samples=FLAGS.num_acquisition_optimiser_start_points)

        rule = EfficientGlobalOptimization(augmented_lagrangian, optimizer=optimizer, num_query_points=FLAGS.batch_size)
