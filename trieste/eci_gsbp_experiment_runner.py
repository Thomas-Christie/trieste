...
INEQUALITY_CONSTRAINT_THREE = "INEQUALITY_CONSTRAINT_THREE"
INEQUALITY_CONSTRAINT_FOUR = "INEQUALITY_CONSTRAINT_FOUR"
INEQUALITY_CONSTRAINT_FIVE = "INEQUALITY_CONSTRAINT_FIVE"
...
flags.DEFINE_string('save_path', 'results/eci_results/gsbp/data/run_', 'Prefix of path to save results to.')
...
def main(argv):
    ...
    for run in range(32, FLAGS.num_experiments):
        ...
        observer = trieste.objectives.utils.mk_multi_observer(
            ...
            INEQUALITY_CONSTRAINT_TWO=constraints.centered_branin,
            INEQUALITY_CONSTRAINT_THREE=constraints.negative_centered_branin,
            INEQUALITY_CONSTRAINT_FOUR=constraints.parr_constraint,
            INEQUALITY_CONSTRAINT_FIVE=constraints.negative_parr_constraint)
        ...
        pof3 = trieste.acquisition.ProbabilityOfFeasibility(threshold=0)
        pof4 = trieste.acquisition.ProbabilityOfFeasibility(threshold=0)
        pof5 = trieste.acquisition.ProbabilityOfFeasibility(threshold=0)
        pof = Product(pof1.using(INEQUALITY_CONSTRAINT_ONE),
                      pof2.using(INEQUALITY_CONSTRAINT_TWO),
                      pof3.using(INEQUALITY_CONSTRAINT_THREE),
                      pof4.using(INEQUALITY_CONSTRAINT_FOUR),
                      pof5.using(INEQUALITY_CONSTRAINT_FIVE))
