...
"""
This module contains the :class:`KKTBayesianOptimizer` class, used to perform Bayesian optimization with the KKT EI
...
from . import logging
from .acquisition.rule import AcquisitionRule, EfficientGlobalOptimization, KKTEfficientGlobalOptimization
...
class KKTBayesianOptimizer(Generic[SearchSpaceType]):
    """
    ...
    def __repr__(self) -> str:
        """"""
        return f"KKTBayesianOptimizer({self._observer!r}, {self._search_space!r})"
    ...
    def optimize(
        ...
        acquisition_rule: KKTEfficientGlobalOptimization,
        acquisition_state: StateType | None = None,
        *,
        track_state: bool = True,
        track_path: Optional[Path | str] = None,
            ...
            try:
                ...
                with Timer() as total_step_wallclock_timer:
                    with Timer() as query_point_generation_timer:
                        points_or_stateful, stop = acquisition_rule.acquire(
                            self._search_space, models, datasets=datasets, initial_alpha=initial_alpha,
                            alpha_lower_bound=alpha_lower_bound
                        )
                        print(f"Stop Optimisation: {stop}")
