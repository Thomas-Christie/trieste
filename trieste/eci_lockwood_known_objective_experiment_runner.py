...
from trieste.models.gpflow import build_gpr, build_eci_lockwood_known_objective_gpr, GaussianProcessRegression
...
def create_known_linear_objective_model(search_space, num_rff_features, data):
    gpr = build_eci_lockwood_known_objective_gpr(data, search_space, likelihood_variance=1e-6)
    return GaussianProcessRegression(gpr, num_rff_features=num_rff_features, use_decoupled_sampler=True)
...
def main(argv):
    ...
    for run in range(FLAGS.num_experiments):
        ...
        initial_models = trieste.utils.map_values(partial(create_model, search_space, FLAGS.num_rff_features, FLAGS.kernel_name),
                                                  initial_data)
        objective_model = create_known_linear_objective_model(search_space, FLAGS.num_rff_features, initial_data["OBJECTIVE"])
        initial_models["OBJECTIVE"] = objective_model
