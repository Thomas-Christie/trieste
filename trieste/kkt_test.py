...
if __name__ == "__main__":
    for run in range(1):
        ...
        kkt_expected_improvement = KKTThompsonSamplingExpectedImprovement(OBJECTIVE, "INEQUALITY", None, 1, search_space, False, False)
        ...
        # with open(f"../results/07-04-23/kkt_lsq_ts/run_{run}_data.pkl", "wb") as fp:
        #     pickle.dump(data, fp)
...
#         initial_models = trieste.utils.map_values(create_model, initial_data)
#
#         kkt_expected_improvement = KKTExpectedImprovement(OBJECTIVE, "INEQUALITY", "EQUALITY", 0.001, search_space, True)
#
#         rule = KKTEfficientGlobalOptimization(kkt_expected_improvement, optimizer=generate_continuous_optimizer(),
#                                               epsilon=EI_EPSILON)
#
...
#         with open(f"../results/07-04-23/multi_run_experiment_two/run_{run}_data.pkl", "wb") as fp:
#             pickle.dump(data, fp)
